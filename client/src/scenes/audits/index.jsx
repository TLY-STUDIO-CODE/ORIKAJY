...
useEffect(() => {
    ...
    const handlePrintLog = () => {
        if (selectedLog) {
            ...
            doc.text(`Utilisateur: ${selectedLog.userId}`, 20, 40);
            doc.text(`Détails: ${selectedLog.details}`, 20, 50);

            doc.save("Données_surveillance.pdf");
    ...
    const columns = [
        ...
        { field: 'timestamp', headerName: 'Date et heure', flex: 1 },
        {
            field: 'actions',
            headerName: 'Actions',
            width: 150,
            renderCell: (params) => (
                <Box>
                    ...
                    <IconButton onClick={() => handleOpenPrintModal(params.row)} color="secondary">
                        <PrintIcon />
                    </IconButton>
                </Box>
            ),
    ...
    return (
        <Box m="20px">
            <Header title="AUDITS" subtitle="Tableau de surveillance de toutes les activités d'un utilisateur dans l'application" />
            <Box
                m="40px 0 0 0"
                height="75vh"
                sx={{
            ...
            <Modal open={openPrintModal} onClose={handleClosePrintModal}>
                ...
                >
                    ...
                    {selectedLog && (
                        <Box>
                            <Typography variant="body1">Identifiant: {selectedLog.id}</Typography>
                            <Typography variant="body1">Action: {selectedLog.action}</Typography>
                            <Typography variant="body1">Utilisateur: {selectedLog.userId}</Typography>
                            <Typography variant="body1">Détails: {selectedLog.details}</Typography>
                        </Box>
                    )}
                    <Button onClick={handlePrintLog} variant="contained" color="secondary" sx={{ mr: 2 }}>
                        Imprimer
                    </Button>
                    <Button onClick={handleClosePrintModal} variant="outlined" color="secondary" >
                        Annuler
                    </Button>
                </Box>
            </Modal>
