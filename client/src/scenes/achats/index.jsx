...
const handleCloseModal = () => {
    ...
    const handleGenerateInvoice = () => {
        if (selectedRowData) {
            ...
            doc.text(`Produit : ${selectedRowData.name}`, 20, 40);
            doc.text(`Description : ${selectedRowData.description}`, 20, 50);
            ...
            doc.text(`Date d'achat : ${selectedRowData.date_achat}`, 20, 100);
            doc.text(`Numéro de facturation : ${selectedRowData.num_factA}`, 20, 110);
            doc.text(`Status : ${selectedRowData.validated}`, 20, 120);

            // Générer et télécharger le fichier PDF
            doc.save("Données_achats.pdf");
        }
    };
const validateAchat = async (achatId) => {
    try {
        ...
        getAchats();
        toast.success("L'achat est validé avec succès!", {
...
const deleteAchat = async (achatId) => {
        try {
            ...
            getAchats();
            toast.success("L'achat est supprimé avec succès!", {
                autoClose: 10000, // 30 seconds
            });
