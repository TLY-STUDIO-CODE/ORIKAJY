...
export const createStock = async (req, res) => {
    ...
    try {
        ...
        io.emit('stock_notification', {
            message: `Le stock de produit ${name} de catégorie ${categories} est ${level} avec une quantité égale à ${qteNumber}!`,
            productName: name,
            stockLevel: qteNumber,
            level: level
        });
    } else  if (qteNumber > 20) {
        // Si la quantité est rétablie à plus de 20
        io.emit('stock_notification', {
            message: `Le stock de produit ${name} de catégorie ${categories} est rempli  avec une quantité de ${qteNumber}!`,
            productName: name,
            stockLevel: qteNumber
        });

        // Émettre l'événement pour supprimer les notifications de stock faible
    
    } else {
        io.emit('clear_notification', { name });
    }
...
export const updateStock = async (req, res) => {
    try {
        ...
        // Si la quantité est rétablie à plus de 20
        io.emit('stock_notification', {
            message: `Le stock de produit ${name} de catégorie ${categories} est revenu à un niveau normal avec une quantité de ${qteNumber}!`,
            productName: name,
            stockLevel: qteNumber
        });
...
export const countStocks = async (req, res) => {
    try {
        const stockCount = await Stock.count(); // Compter tous les achat
        res.status(200).json({ totalStocks: stockCount });
    } catch (error) {
        res.status(500).json({ msg: error.message });
    }
};