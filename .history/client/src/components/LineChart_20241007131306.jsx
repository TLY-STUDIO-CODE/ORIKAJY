      ...
      if (date) { // Check if a date is selected
        ...
        if (financialData) {
          const formattedData = [
            {
              id: "Dépenses",
              color: tokens("dark").greenAccent[500], // Green for Expenses
              data: [{ x: date, y: financialData.depenses }],
            },
            {
              id: "Bénéfice",
              color: tokens("dark").blueAccent[300], // Blue for Profit
              data: [{ x: date, y: financialData.benefice }],
            },
            {
              id: "Perte",
              color: tokens("dark").redAccent[200], // Red for Loss
    ...
    <>
      ...
      <ResponsiveLine
        ...
        axisBottom={{
          ...
          tickValues: lineData.map((data) => data.data[0].x), // Show specific dates on the x-axis
        }}
        axisLeft={{
          orient: "left",
          tickValues: 5,
        ...
        pointLabel={e => `${e.y}` } // Show Y value on hover
        enableSlices="x" // Enable vertical slices for hover effects
        tooltip={({ point }) => (
          <div style={{ padding: "10px", background: colors.primary[500], borderRadius: "5px", color: 'white' }}>
            <strong>{point.serieId}</strong><br />
            <span>Date: {point.data.x}</span><br />
            <span>Montant: {point.data.y}</span><br />
          </div>
        )} // Custom tooltip for detailed data display
      />
    </>
  );
};
