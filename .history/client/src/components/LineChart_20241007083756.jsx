import { useState, useEffect } from "react";
import axios from "axios";
import { ResponsiveLine } from "@nivo/line";
import { useTheme } from "@mui/material";
import { tokens } from "../theme";
import { toast } from "react-toastify";

const LineChart = ({ isCustomLineColors = false, isDashboard = false }) => {
  const theme = useTheme();
  const colors = tokens(theme.palette.mode);
  
  // Ajout des états pour les différentes données
  const [selectedDate, setSelectedDate] = useState("");
  const [revenuTotal, setRevenuTotal] = useState(0);
  const [revenuByDate, setRevenuByDate] = useState(0);
  const [totalVenteByDate, setTotalVenteByDate] = useState(0);
  const [totalVentes, setTotalVentes] = useState(0);
  const [depenseTotal, setDepenseTotal] = useState(0);
  const [depenseByDate, setDepenseByDate] = useState(0);
  const [pertes, setPertes] = useState(0);
  const [pertesByDate, setPertesByDate] = useState(0);
  const [beneficeTotal, setBeneficeTotal] = useState(0);
  const [beneficeByDate, setBeneficeByDate] = useState(0);

  // Fonction pour récupérer les données
  const fetchAllData = async () => {
    try {
      const [ventesResponse, depensesResponse, revenusResponse, pertesResponse, beneficesResponse] = await Promise.all([
        axios.get(`http://localhost:5000/ventes/${selectedDate || ""}`),
        axios.get(`http://localhost:5000/depenses/${selectedDate || ""}`),
        axios.get(`http://localhost:5000/revenus/${selectedDate || ""}`),
        axios.get(`http://localhost:5000/pertes/${selectedDate || ""}`),
        axios.get(`http://localhost:5000/benefice/${selectedDate || ""}`)
      ]);

      setTotalVentes(ventesResponse.data.totalVentes);
      setDepenseTotal(depensesResponse.data.totalDepenses);
      setRevenuTotal(revenusResponse.data.revenuTotal);
      setBeneficeTotal(beneficesResponse.data.benefice);
      setPertes(pertesResponse.data.pertes);
    } catch (error) {
      toast.error("Erreur lors de la récupération des données.");
    }
  };

  // Utilisation de useEffect pour appeler fetchAllData quand la date change
  useEffect(() => {
    fetchAllData();
  }, [selectedDate]);

  // Données du graphique
  const data = [
    {
      id: "Revenu",
      color: colors.primary[500],
      data: [
        { x: "Date", y: revenuTotal },
        { x: "Vente", y: totalVenteByDate },
      ],
    },
    {
      id: "Dépense",
      color: colors.secondary[500],
      data: [
        { x: "Date", y: depenseTotal },
        { x: "Pertes", y: pertesByDate },
      ],
    },
    {
      id: "Bénéfice",
      color: colors.tertiary[500],
      data: [
        { x: "Date", y: beneficeTotal },
      ],
    },
  ];

  return (
    <ResponsiveLine
      data={data}
      theme={{
        axis: {
          domain: {
            line: {
              stroke: colors.grey[100],
            },
          },
          legend: {
            text: {
              fill: colors.grey[100],
            },
          },
          ticks: {
            line: {
              stroke: colors.grey[100],
              strokeWidth: 1,
            },
            text: {
              fill: colors.grey[100],
            },
          },
        },
        legends: {
          text: {
            fill: colors.grey[100],
          },
        },
        tooltip: {
          container: {
            color: colors.primary[500],
          },
        },
      }}
      colors={isDashboard ? { datum: "color" } : { scheme: "nivo" }}
      margin={{ top: 50, right: 110, bottom: 50, left: 60 }}
      xScale={{ type: "point" }}
      yScale={{
        type: "linear",
        min: "auto",
        max: "auto",
        stacked: true,
        reverse: false,
      }}
      yFormat=" >-.2f"
      curve="catmullRom"
      axisTop={null}
      axisRight={null}
      axisBottom={{
        orient: "bottom",
        tickSize: 0,
        tickPadding: 5,
        tickRotation: 0,
        legend: isDashboard ? undefined : "Données",
        legendOffset: 36,
        legendPosition: "middle",
      }}
      axisLeft={{
        orient: "left",
        tickValues: 5,
        tickSize: 3,
        tickPadding: 5,
        tickRotation: 0,
        legend: isDashboard ? undefined : "Valeurs",
        legendOffset: -40,
        legendPosition: "middle",
      }}
      enableGridX={false}
      enableGridY={false}
      pointSize={8}
      pointColor={{ theme: "background" }}
      pointBorderWidth={2}
      pointBorderColor={{ from: "serieColor" }}
      pointLabelYOffset={-12}
      useMesh={true}
      legends={[
        {
          anchor: "bottom-right",
          direction: "column",
          justify: false,
          translateX: 100,
          translateY: 0,
          itemsSpacing: 0,
          itemDirection: "left-to-right",
          itemWidth: 80,
          itemHeight: 20,
          itemOpacity: 0.75,
          symbolSize: 12,
          symbolShape: "circle",
          symbolBorderColor: "rgba(0, 0, 0, .5)",
          effects: [
            {
              on: "hover",
              style: {
                itemBackground: "rgba(0, 0, 0, .03)",
                itemOpacity: 1,
              },
            },
          ],
        },
      ]}
    />
  );
};

export default LineChart;
