...
const LineChart = ({ selectedDate }) => {
  ...
  const fetchChartData = async () => {
    if (!selectedDate) return; // Ne pas tenter de récupérer les données si aucune date n'est sélectionnée
      ...
      setChartData([
        {
          ...
          data: ventesResponse.data.map(item => ({
            x: item.date, // Remplacez 'date' par le nom correct de la propriété dans la réponse
            y: item.totalVentes // Remplacez 'totalVentes' par le nom correct
          })),
        },
        {
          id: "Dépenses",
          data: depensesResponse.data.map(item => ({
            x: item.date, // Remplacez 'date' par le nom correct
            y: item.totalDepenses // Remplacez 'totalDepenses' par le nom correct
          })),
        },
        {
          id: "Revenus",
          data: revenusResponse.data.map(item => ({
            x: item.date, // Remplacez 'date' par le nom correct
            y: item.revenuTotal // Remplacez 'revenuTotal' par le nom correct
          })),
        },
        {
          id: "Pertes",
          data: pertesResponse.data.map(item => ({
            x: item.date, // Remplacez 'date' par le nom correct
            y: item.pertes // Remplacez 'pertes' par le nom correct
          })),
        },
        {
          id: "Bénéfice",
          data: beneficesResponse.data.map(item => ({
            x: item.date, // Remplacez 'date' par le nom correct
            y: item.benefice // Remplacez 'benefice' par le nom correct
          })),
        }
      ]);
    } catch (error) {
