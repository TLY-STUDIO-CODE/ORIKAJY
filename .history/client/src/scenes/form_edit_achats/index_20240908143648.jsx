...
useEffect(()=>{
    const getAchatById = async () =>{
        try {
            ...
            setName_four(response.data.name_four);
            setQte(response.data.qte);
            setCategories(response.data.categories);
            setDescription(response.data.description);
            setMontant(response.data.montant);
            setDate_achat(response.data.date_achat);
            setNum_factA(response.data.num_factA);
        } catch (error) {
            if(error.response) {
...
const updateAchat = async(e) =>{
    e.preventDefault();
    try {
        await axios.patch(`http://localhost:5000/achats/${id}`, {
            name: name,
            name_four: name_four,
    ...
    return (
        <Box m="20px">
            <Header title="METTRE A JOUR STOCK" subtitle="Mise à jour d'un achat de produit" />
            ...
            >
                {({
                    ...
                    <Box display="flex" justifyContent="end" mt="20px">
                        <Button type="submit" color="secondary" variant="contained" onClick={updateAchat}>
                            Créer
                        </Button>
                    </Box>
                    {msg && <p>{msg}</p>}
...
num_factA: yup.string().required("required"),
