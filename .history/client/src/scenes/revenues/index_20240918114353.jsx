...
const Revenues = () => {
    ...
    const fetchRevenues = async () => {
        try {
            const url = searchDate ?  'http://localhost:5000/revenus';
            console.log(`Fetching data from URL: ${url}`); // Log the URL
            const response = await axios.get(url);
            console.log('Response:', response.data); // Log the response data
            setRevenues(response.data);
        } catch (error) {
            console.error("Error fetching revenues:", error); // Log the error
    ...
    const columns = [
        ...
        },
        {
            ...
            renderCell: (params) => (
                <Box>
                    <IconButton 
                        onClick={() => toast.error("Fonction de suppression non implémentée")} 
                        color="secondary"
                    >
                        <DeleteIcon />
                    </IconButton>
