...
const Revenues = () => {
    ...
    const [perteByDate, setPerteByDate] = useState(0);
    ...
    const handleFetchPerteTotal = async () => {
        try {
            ...
            setPerteTotal(response.data.perteTotal);
        } catch (error) {
            toast.error("Erreur lors de la récupération des pertes.");
        }
    };

    // Fonction pour récupérer la perte par date
    const handleFetchPerteByDate = async () => {
        try {
            const response = await axios.get(`http://localhost:5000/pertes/${selectedDate}`);
            setPerteByDate(response.data.perteByDate);
        } catch (error) {
    ...
    return (
        <Box m="20px">
            ...
            <Box mt="20px">
                ...
                <Button
                    onClick={handleFetchPerteByDate}
                    variant="outlined"
                    color="error"
                    sx={{ ml: 2 }}
                >
            ...
            <Box mt="20px">
                ...
                <Typography variant="h6">Total Perte par date: {perteByDate} AR</Typography>
                <Typography variant="h6">Total Perte: {perteTotal} AR</Typography>
            </Box>
