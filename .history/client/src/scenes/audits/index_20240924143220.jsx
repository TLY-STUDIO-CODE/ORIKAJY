...
const AuditAdmin = () => {
    ...
    useEffect(() => {
        const fetchLogs = async () => {
            try {
                const loginResponse = await axios.get('http://localhost:5000/audits/logins');
                const activityResponse = await axios.get('http://localhost:5000/audits/activities');
    ...
    const columns = [
        ...
        { field: 'timestamp', headerName: 'Timestamp', flex: 1 },
        {
            field: 'delete', headerName: 'Supprimer', flex: 1, renderCell: (params) => (
                <Button variant="contained" color="secondary" onClick={() => deleteLog(params.id)}>
                    Supprimer
                </Button>
            )
        }
    ...
    return (
        <Box m="20px">
            ...
            <Typography variant="h4" gutterBottom>
                Logs d'Audit - Activités des utilisateurs
            </Typography>
