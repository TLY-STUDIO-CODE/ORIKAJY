...
const [items, setItems] = useState([{ name: "", description: "", categories: "", qte: "", montant: "", unit: "" }]);
...
const addItem = () => {
if (items.length < 10) {
setItems([...items, { name: "", description: "", categories: "", qte: "", montant: "", unit: "" }]);
} else {
...
const saveVente2 = async (e) => {
e.preventDefault();
if (items.length < 1) {
setMsg("Vous devez vendre au moins un produit.");
return;
}
try {
await axios.post('http://localhost:5000/ventes2', {
items,
name_client,
date_vente,
num_factV
});
toast.success("Nouvelle vente créé avec succès!", {
...
return (
<Box m="20px">
<Header title="NOUVELLE VENTE" subtitle="Formulaire pour Vendre un produit" />
<Formik
...
type="number"
label="Prix unitaire"
onChange={(e) => handleItemChange(index, 'montant', e.target.value)}
value={item.montant}
sx={{ gridColumn: "span 2" }}
/>
...
const initialValues = {
items: [
{ name: "", description: "", categories: "", qte: "", montant: "" } // Valeurs initiales pour chaque élément
],
name_client: "",
date_vente: "",
num_factV: ""
