    ...
    const getCommandes = async () => {
        try {
            ...
            setCommandes(groupedCommandes);
             // Si vous voulez récupérer montant_totalC du premier client
        if (groupedCommandes.length > 0) {
            setMontantTotal(groupedCommandes[0].montant_totalC);
        }
        } catch (error) {
...
<Box m="20px">
    ...
    <Header title="TABLEAU DE BORD USER" subtitle="Bienvenue" name={user && user.name}  />
    ...
    >
        <StatBox
        title={`Montant Total: ${montantTotal}`}  // Mettez à jour ici
        subtitle="Commandes"
        progress="0.75"
        increase="+14%"
        icon={
