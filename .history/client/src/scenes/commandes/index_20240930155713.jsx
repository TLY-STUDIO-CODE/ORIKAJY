...
const [selectedCommande, setSelectedCommande] = useState(null);
    ...
    const handleOpenDeleteModal = (commande) => {
...
const handleCloseDeleteModal = () => {
    ...
    const handleGenerateInvoice = () => {
        if (selectedRowData) {
            ...
            doc.text(`Prix unitaire : ${selectedRowData.montants.join(", ")} AR`, 20, 90);
            doc.text(`Prix total: ${selectedRowData.montant_totalC} AR`, 20, 100);
            doc.text(`Date de commande : ${selectedRowData.date_commande}`, 20, 110);
            doc.text(`Numéro de facturation : ${selectedRowData.num_factC}`, 20, 120);
            doc.text(`Status : ${selectedRowData.validated}`, 20, 130);

            // Générer et télécharger le fichier PDF
            doc.save("Données_commandes.pdf");
        }
    };
    ...
    const deleteCommande = async (commandeId) => {
        try {
            ...
            getCommandes();
            toast.success("La commande est supprimée avec succès!", {
    ...
    try {
        ...
        getCommandes();
        toast.success("La commande est validée avec succès!", {
            autoClose: 10000, // 30 seconds
        });
    } catch (error) {
    ...
    const printInvoice = (commande) => {
        ...
        printWindow.document.write(`<p>Déscription : ${commande.descriptions.join(", ")}</p>`);
        printWindow.document.write(`<p>Quantité : ${commande.qtes.join(", ")}</p>`);
        printWindow.document.write(`<p>Unité : ${commande.units.join(", ")}</p>`);
        printWindow.document.write(`<p>Catégorie : ${commande.categories.join(", ")}</p>`);
        printWindow.document.write(`<p>Prix unitaire : ${commande.montants.join(", ")} AR</p>`);
        printWindow.document.write(`<p>Prix total :  ${commande.montant_totalC} AR</p>`);
        ...
        <Box m="20px">
            ...
            <Modal open={openDeleteModal} onClose={handleCloseDeleteModal}>
                ...
                >
                    ...
                    <Box mt={2}>
                        <Button onClick={() => {
                            deleteCommande(selectedCommande.uuid);
                ...
                >
                    ...
                    {selectedRowData && (
                        <Box>
                            ...
                            <Typography variant="body1">Déscription: {selectedRowData.descriptions.join(", ")}</Typography>
                            ...
                            <Typography variant="body1">Prix total: {selectedRowData.montant_totalC} AR</Typography>
                            <Typography variant="body1">Date de commande: {selectedRowData.date_commande}</Typography>
