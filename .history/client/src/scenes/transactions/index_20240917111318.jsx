...
// Fonction pour regrouper les ventes par num_factT
    ...
    const handleGenerateInvoice = () => {
        if (selectedRowData) {
            ...
            doc.text(`Descriptions de transaction: ${selectedRowData.descriptions.join(", ")}`, 20, 40);
            doc.text(`Quantité de produit : ${selectedRowData.qtes.join(", ")}`, 20, 50);
            doc.text(`Type de transaction: ${selectedRowData.types.join(", ")}`, 20, 60);
            doc.text(`Prix unitaire de produit: ${selectedRowData.montants.join(", ")}`, 20, 70);
            doc.text(`Date de transaction: ${selectedRowData.date_transaction}`, 20, 80);
            doc.text(`Numéro de facturation: ${selectedRowData.num_factT}`, 20, 90);
            doc.text(`Total Montant: ${selectedRowData.montant_total_transaction} ar`, 20, 100);
                ...
                >
                    ...
                    {selectedRowData && (
                        <Box>
                            ...
                            <Typography variant="body1">Total prix: {selectedRowData.montant_total_transaction} €</Typography>
