...
// Fonction pour regrouper les ventes par num_factT
    ...
    const handleGenerateInvoice = () => {
        if (selectedRowData) {
            ...
            doc.text(`Descriptions de transaction: ${selectedRowData.descriptions.join(", ")}`, 20, 30);
            doc.text(`Quantité de produit : ${selectedRowData.qtes.join(", ")}`, 20, 40);
            doc.text(`Type de transaction: ${selectedRowData.types.join(", ")}`, 20, 50);
            doc.text(`Prix unitaire de produit: ${selectedRowData.montants.join(", ")} AR`, 20, 60);
            doc.text(`Date de transaction: ${selectedRowData.date_transaction}`, 20, 70);
            doc.text(`Numéro de facturation: ${selectedRowData.num_factT}`, 20, 80);
            doc.text(`Total Prix: ${selectedRowData.montant_total} AR`, 20, 90);
...
return (
<Box m="20px">
<Header title="TRANSACTIONS" subtitle="Liste de tous les transactions" />
<Box
    m="40px 0 0 0"
    height="75vh"
    sx={{
    ...
    >
        ...
        {selectedRowData && (
            <Box>
                ...
                <Typography variant="body1">Types: {selectedRowData.types.join(", ")}</Typography>
