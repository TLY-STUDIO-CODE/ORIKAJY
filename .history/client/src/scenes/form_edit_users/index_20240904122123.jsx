...
import { Box, Button, TextField, IconButton, InputAdornment, MenuItem, Typography, useTheme } from "@mui/material";
...
const EditUser = () => {
    ...
    const { id } = useParams(); // Ajout de useParams pour obtenir l'id de l'utilisateur
    ...
    const [imagePreview, setImagePreview] = useState(""); // Prévisualisation de l'image
    ...
    useEffect(() => {
        const getUserById = async () => {
            try {
                ...
                setImagePreview(response.data.image); // Mettre à jour l'URL de l'image
            } catch (error) {
                if (error.response) {
    ...
    return (
        <Box m="20px">
            <Header title="METTRE A JOUR" subtitle="Mettre à jour un utilisateur" />
                ...
                {({
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <Box display="flex" justifyContent="end" mt="20px">
                            <Button type="submit" color="secondary" variant="contained" onClick={updateUser}>
                                Mettre à jour
                            </Button>
                        </Box>
                        {msg && <p>{msg}</p>}
