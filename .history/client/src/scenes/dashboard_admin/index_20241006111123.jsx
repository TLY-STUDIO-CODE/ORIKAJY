...
const totalCommandesMax = 500;

  // Calcul du pourcentage en fonction du nombre de commandes
  const progressPercentage = Math.min(commandeCount / totalCommandesMax, 1); // 1 correspond à 100%
  const revenueGenerated = commandeCount * 96.70; // Vous pouvez ajuster cette valeur selon vos besoins

  // Couleur du cercle : vert si des commandes existent, sinon bleu
  const progressColor = commandeCount >= totalCommandesMax ? colors.greenAccent[500] : colors.blueAccent[500];
    useEffect(() => {
        dispatch(getMe());
    }, [ dispatch]);
      ...
      >
        ...
        >
          ...
          <Box
            ...
            mt="25px"
          >
            <ProgressCircle progress={progressPercentage} size="125" progressColor={progressColor} />
            <Typography
              variant="h5"
              color={colors.greenAccent[500]}
              sx={{ mt: "15px" }}
            >
              {revenueGenerated.toLocaleString()}€ revenue generated
            </Typography>
            <Typography>{commandeCount} commandes sur un total de 500</Typography>
