...
const handleCloseDeleteModal = () => {
    ...
    const handleGenerateInvoice = () => {
        if (selectedRowData) {
            const doc = new jsPDF();
            // Titre de la facture
            doc.text("DONNEES DE STOCKAGE ", 20, 10);
            doc.text(`Créé par : ${selectedRowData.createdBy}`, 20, 20);
            doc.text(`Produit : ${selectedRowData.name}`, 20, 30);
            doc.text(`Quantité : ${selectedRowData.qte}`, 20, 40);
            doc.text(`Unité : ${selectedRowData.unit}`, 20, 50);
            doc.text(`Catégorie : ${selectedRowData.categories}`, 20, 60);
            doc.text(`Prix unitaire : ${selectedRowData.montantS} AR`, 20, 70);
            doc.text(`Date de mise en stock : ${selectedRowData.date_stock}`, 20, 80);
            doc.text(`Numéro de facturation : ${selectedRowData.num_fact}`, 20, 90);
            doc.text(`Prix total : ${selectedRowData.montant_totalS} AR`, 20, 100);

            // Générer et télécharger le fichier PDF
            doc.save("Données_stocks.pdf");
        }
    };
    ...
    const deleteStock = async (stockId) => {
        try {
            ...
            getStocks();
            toast.success("Le stock est supprimé avec succès!", {
...
return (
<Box m="20px">
<Header title="STOCKS" subtitle="Liste de tous les stocks de produit" />
<Box
    m="40px 0 0 0"
    height="75vh"
    sx={{
...
{/* Modal de confirmation de suppression */}
            <Modal open={openDeleteModal} onClose={handleCloseDeleteModal}>
                ...
                >
                    ...
                    <Box mt={2}>
                        <Button onClick={() => {
                            deleteStock(selectedStock.uuid);
                ...
                >
                    ...
                    {selectedRowData && (
                        <Box>
                            ...
                            <Typography variant="body1">Qunatité : {selectedRowData.qte}</Typography>
                            <Typography variant="body1">Unité : {selectedRowData.unit}</Typography>
                            <Typography variant="body1">Prix unitaire : {selectedRowData.montantS} AR</Typography>
                            <Typography variant="body1">Prix total: {selectedRowData.montant_totalS} AR</Typography>
