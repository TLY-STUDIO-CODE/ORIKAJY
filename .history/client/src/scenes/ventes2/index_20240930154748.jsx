...
const handleGenerateInvoice = () => {
    if (selectedRowData) {
        const doc = new jsPDF();
            ...
            doc.text(`Prix total : ${selectedRowData.montant_total} AR`, 20, 100);
            doc.text(`Date de vente : ${selectedRowData.date_vente}`, 20, 110);
            doc.text(`Numéro de facturation : ${selectedRowData.num_factV}`, 20, 120);
            doc.text(`Status : ${selectedRowData.validated}`, 20, 130);
            // Générer et télécharger le fichier PDF
            doc.save("Données_ventes.pdf");
        }
    };
    ...
    try {
        ...
        getVentes2();
        toast.success("La vente est validée avec succès!", {
    ...
    const deleteVente2 = async (venteId) => {
        try {
            ...
            getVentes2();
            toast.success("La vente est supprimée avec succès!", {
                autoClose: 10000, // 30 seconds
            });
    ...
    const printInvoice = (vente) => {
        ...
        printWindow.document.write(`<p>Prix unitaire : ${vente.montants.join(", ")} AR</p>`);
