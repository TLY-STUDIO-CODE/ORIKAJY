...
const handleOpenModal = (rowData) => {
    ...
    const handleGenerateInvoice = () => {
        if (selectedRowData) {
            const doc = new jsPDF();
            // Titre de la facture
            doc.text(`DONNEES D'ACHAT CREE PAR:${selectedRowData.createdBy}`, 20, 10);
            doc.text(`Fourniseur: ${selectedRowData.name_four}`, 20, 20);
            doc.text(`Produits: ${selectedRowData.name}`, 20, 30);
            doc.text(`Descriptions de achat: ${selectedRowData.description}`, 20, 40);
            doc.text(`Quantité de produit : ${selectedRowData.qte}`, 20, 50);
            doc.text(`Catégorie de produit: ${selectedRowData.categories}`, 20, 60);
            doc.text(`Prix unitaire de produit: ${selectedRowData.montant} AR`, 20, 70);
            doc.text(`Date de achat: ${selectedRowData.date_achat}`, 20, 80);
            doc.text(`Numéro de facturation: ${selectedRowData.num_factA}`, 20, 90);
...
return (
<Box m="20px">
<Header title="LISTE DES ACHATS" subtitle="Liste de tous les achats de produit" />
<Box
    m="40px 0 0 0"
    height="75vh"
    sx={{
...
{/* Modal pour générer la facture */}
            <Modal open={openModal} onClose={handleCloseModal}>
                ...
                >
                    ...
                    {selectedRowData && (
                        <Box>
                            <Typography variant="body1">Créer par: {selectedRowData.createdBy}</Typography>
