...
export const Login = async (req, res) => {
    try {
        ...
        await logAudit("Connexion", user.name, `Utilisateur connecté avec l'adresse email : ${user.email}`);
...
export const logOut = async (req, res) => {
    ...
    try {
        ...
        // Enregistrer l'action dans les logs d'audit
        await logAudit("Déconnexion", req.session.userId == user.name, `Utilisateur déconnecté : ${req.session.userId}`);

        req.session.destroy((err) => {
            if (err) return res.status(400).json({ msg: "Impossible de se déconnecter" });
            res.status(200).json({ msg: "Vous êtes déconnecté", name });
        });
    } catch (error) {
        res.status(500).json({ msg: "Erreur serveur" });
    }
