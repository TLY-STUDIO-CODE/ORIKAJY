...
export const getStocks = async (req, res) => {
    try {
        ...
        // Notify clients of low stock in real-time
        const lowStockItems = response.filter(item => item.qte <= 10);
        const veryLowStockItems = response.filter(item => item.qte <= 5);

        io.emit('stockUpdate', { lowStockItems, veryLowStockItems });
