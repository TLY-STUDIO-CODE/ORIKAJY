...
export const updateStock = async (req, res) => {
    try {
        ...
        const { v4: uuidv4 } = require('uuid'); // Assurez-vous d'importer uuid

if (qteNumber <= 20) {
    const level = qteNumber <= 10 ? 'Très faible' : 'Faible';
    const notificationId = uuidv4(); // Générer un UUID

    io.emit('stock_notification', {
        id: notificationId, // Ajouter l'UUID ici
        message: `Stock de ${name} de catégorie ${categories} est ${level} avec une quantité égale à ${qteNumber}`,
        productName: name,
    ...
    io.emit('clear_notification', { id: notificationId }); // Inclure l'UUID ici
}
