...
export const getUsers = async (req, res) => {
    try {
        ...
        const usersWithStatus = response.map(user => ({
            ...
            // Vérifier si l'utilisateur est le même que celui de la session active
            status: currentUserId === user.uuid ? "connecté" : "déconnecté"  
        }));

        res.status(200).json(usersWithStatus);
    } catch (error) {
        console.error(error);  // Log pour déboguer plus facilement
...
export const getUserById = async (req, res) => {
    try {
        ...
        const userWithStatus = {
            ...response.toJSON(),  // Convertir en JSON pour ajouter un champ
            status: currentUserId === response.uuid ? "connecté" : "déconnecté"
        };
