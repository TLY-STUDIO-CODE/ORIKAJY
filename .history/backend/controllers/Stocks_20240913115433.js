...
export const createStock = async (req, res) =>{
    ...
    try {
        ...
        let montant_totalS = qteNumber * montantS; // Calcul du montant_total
        // Adjust montant based on the unit
        if (unit === 'kilo') {
            montant_totalS = qteNumber * montantS; // Assuming montant is per kilo
        } else if (unit === 'litre') {
            montant_totalS = qteNumber * montantS; // Assuming montant is per litre
        } else if (unit === 'gramme') {
            montant_totalS = (qteNumber / 1000) * montantS; // Assuming montant is per kilo, so dividing by 1000
        } else if (unit == 'unité') {
            montant_totalS = qteNumber * montantS; // Assuming montant is per unite
        }
        totalStock += montant_totalS;
        await Stock.create({
            name,
            qte: qteNumber,
            unit,
...
export const updateStock = async (req, res) =>{
    try {
        ...
        const qteNumber = parseFloat(qte); // Convertir qte en nombre
        let montant_totalS = qte * montantS; // Calcul du montant_total
        // Adjust montant based on the unit
        if (unit === 'kilo') {
            montant_totalS = qteNumber * montantS; // Assuming montant is per kilo
        } else if (unit === 'litre') {
