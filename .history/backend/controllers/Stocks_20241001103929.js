...
let io; // Déclarez io en dehors de votre export
export const setSocketIo = (socketIo) => {
    io = socketIo; // Assignez-le lorsque vous initialisez votre serveur
};
...
export const createStock = async (req, res) => {
    ...
    try {
        ...
        notifyStockLevel(Stock.qte, Stock.name);
...
export const updateStock = async (req, res) => {
    try {
        ...
        notifyStockLevel(qteNumber, name);
         // Log the validation action
        const user = await User.findOne({
            where: {
                uuid: req.session.userId
...
const notifyStockLevel = (qte, name) => {
    if (qte <= 10) {
        io.emit("stockNotification", { message: `Stock très faible pour ${name}` });
    } else if (qte <= 20) {
        io.emit("stockNotification", { message: `Stock faible pour ${name}` });
    } else {
        io.emit("stockNotification", { message: `Niveau de stock rétabli pour ${name}` });
    }
};
