    ...
    try {
        ...
        if (req.role === "admin") {
            response = await Transaction.findAll({
                attributes: ['uuid', 'description_transaction', 'montant_transaction', 'type_transaction', 'date_transaction', 'userId'],
                include: [{
                    model: User,
...
export const createTransaction = async (req, res) => {
    ...
    try {
        ...
        // Trigger notification for new transaction
        sendTransactionNotification(req.userId, type);
    } catch (error) {
        res.status(500).json({ msg: error.message });
    }
}
...
export const sendTransactionNotification = (userId, type) => {
    console.log(`Nouvelle transaction effectuée par l'utilisateur: ${userId}, Type de transaction: ${type}`);
    // Add your logic here to notify the user (email, message, etc.)
}
