...
import { broadcastStockUpdate } from "../index.js"; 

const LOW_STOCK_THRESHOLD = 10; // Seuil de stock faible
const VERY_LOW_STOCK_THRESHOLD = 5; // Seuil de stock très faible

const checkStockLevels = async () => {
    const lowStockItems = await Stock.findAll({
        where: {
            qte: { [Op.lt]: LOW_STOCK_THRESHOLD }
        }
    });

    const veryLowStockItems = await Stock.findAll({
        where: {
            qte: { [Op.lt]: VERY_LOW_STOCK_THRESHOLD }
        }
    });

    // Émettre des notifications si des articles de stock faible ou très faible sont trouvés
    if (lowStockItems.length > 0 || veryLowStockItems.length > 0) {
        broadcastStockUpdate(lowStockItems, veryLowStockItems);
    }
};
...
// Appeler checkStockLevels lors de l'initialisation de votre application
setInterval(checkStockLevels, 60000);