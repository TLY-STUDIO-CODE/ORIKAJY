...
export const Login = async (req, res) => {
    try {
        ...
        if (!user) {
            await logAudit("Login", "guest", `Tentative de connexion avec un email introuvable : ${req.body.email}`);
            return res.status(404).json({ msg: "Utilisateur introuvable" });
        }

        const match = await argon2.verify(user.password, req.body.password);
        if (!match) {
            await logAudit("Login", user.uuid, `Tentative de connexion échouée (mot de passe incorrect) pour l'utilisateur ${user.email}`);
        ...
        await logAudit("Login", user.uuid, `Utilisateur connecté : ${user.email}`);
