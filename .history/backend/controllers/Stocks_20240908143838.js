    ...
    try {
        ...
        if(req.role === "admin"){
            response = await Stock.findOne({
                attributes:['uuid', 'name', 'name_four', 'qte', 'categories', 'montantS', 'montant_totalS', 'date_stock', 'num_fact'],
                where:{
                    id: stock.id
            },
                include:[{
...
export const createStock = async (req, res) =>{
    const {name, name_four, qte, categories, montantS, date_stock, num_fact} = req.body;
    try {
...
export const updateStock = async (req, res) =>{
    try {
        ...
        const montant_totalS = qte * montantS; // Calcul du montant_total
        if(req.role === "admin"){
            await Stock.update({name, name_four, qte, categories, montantS, montant_totalS, date_stock, num_fact},{
                where:{
                    id: stock.id
                }
            });
