...
// Ajouter une variable globale io pour socket
let io;
...
export const logAudit = async (action, userId, details) => {
    try {
        const newAudit = await Audit.create({ action, userId, details, timestamp: new Date() });
        if (io) {
            io.emit("newAudit", newAudit); // Émettre l'événement audit
        }
    } catch (error) {
...
export const deleteAudit = async (req, res) => {
    ...
    try {
        ...
        await audit.destroy();
        if (io) {
            io.emit("deleteAudit", { id }); // Émettre l'événement de suppression
        }
