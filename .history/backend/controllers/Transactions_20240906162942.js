...
export const updateTransaction = async (req, res) =>{
    try {
        ...
        if(!transaction) return res.status(404).json({msg: "Données introuvables"});
        const {description_transaction, type_transaction, montant_transaction, date_transaction} = req.body;
        if(req.role === "admin"){
            await Transaction.update({description_transaction, type_transaction, montant_transaction, date_transaction},{
                where:{
                    id: transaction.id
                }
            });
        ...
        res.status(200).json({msg: "Transaction mis à jour avec succès"});
    } catch (error) {
        res.status(500).json({msg: error.message});
    }
}
