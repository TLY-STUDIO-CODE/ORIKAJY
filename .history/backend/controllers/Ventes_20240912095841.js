...
export const createVente = async (req, res) => {
    ...
    try {
        ...
        for (let item of items) {
            const { name, description, categories, qte, montant } = item;
            ...
            const montant_total = qteNumber * montant;
            totalVente += montant_total;

            // Création de la vente
            await Vente.create({
                name, description, categories, qte: qteNumber, montant, montant_total, name_client, date_vente, num_factV, stockId: stock.id, userId: req.userId
            });
            ...
            await Transaction.create({
                description_transaction: `Vente de ${name} - ${description} - ${name_client}`,
        ...
        res.status(201).json({ msg: "Vente créée et stock mis à jour avec succès", totalVente });
    } catch (error) {
        res.status(500).json({ msg: error.message });
    }
};
...
export const deleteVente = async (req, res) => {
    try {
        const vente= await Vente.findOne({ where: { uuid: req.params.id } });
        if (!vente) return res.status(404).json({ msg: "Données introuvables" });
        const qteNumber = parseFloat(vente.qte); // Convertir qte en nombre
