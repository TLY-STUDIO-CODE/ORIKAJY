...
export const Login = async (req, res) => {
    try {
        ...
        if (!match) {
            await logAudit("Connexion", user.name, `Tentative de connexion échouée (mot de passe incorrect) pour l'utilisateur ${user.email}`);
        ...
        await logAudit("Connexion", user.name, `Utilisateur avec l'adresse email ${user.email} est connecté`);
...
export const logOut = async (req, res) => {
    ...
    try {
        ...
        // Enregistrer l'action dans les logs d'audit
        await logAudit("Déconnexion", user.name, `Utilisateur avec l'adresse email ${user.email} est déconnecté `);
