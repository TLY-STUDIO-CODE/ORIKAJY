    ...
    try {
        ...
        if(req.role === "admin"){
            response = await Stock.findAll({
                attributes:['uuid', 'name', 'name_four', 'qte', 'categories', 'num_fact'],
                include:[{
                    model: User, 
...
export const createStock = async (req, res) =>{
    const {name, name_four, qte, categories, num_fact} = req.body;
    try {
        await Stock.create({
            name: name,
            name_four: name_four,
...
export const updateStock = async (req, res) =>{
    try {
        ...
        const {name, name_four, qte, categories, num_fact} = req.body;
        if(req.role === "admin"){
            await Stock.update({name, name_four, qte, categories, num_fact},{
                where:{
                    id: stock.id
                }
            });
...
if (veryLowStockItems.length > 0 || lowStockItemsEqualTen.length > 0) {
    io.emit("stockNotification", {
    msg: "Notifications de stock",
