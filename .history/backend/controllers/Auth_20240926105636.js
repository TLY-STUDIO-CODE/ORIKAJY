...
export const Login = async (req, res) => {
    try {
        ...
        if (!user) {
            await logAudit("Se connecter", "guest", `Tentative de connexion avec un email introuvable : ${req.body.email}`);
            return res.status(404).json({ msg: "Utilisateur introuvable" });
        }

        const match = await argon2.verify(user.password, req.body.password);
        if (!match) {
            await logAudit("Se connecter", user.name, `Tentative de connexion échouée (mot de passe incorrect) pour l'utilisateur ${user.email}`);
        ...
        await logAudit("Se connecter", user.name, `Email : ${user.email} et Status : connecté `);
...
export const logOut = async (req, res) => {
    ...
    try {
        ...
        // Enregistrer l'action dans les logs d'audit
        await logAudit("Se déconnecter", user.name, `Email : ${user.email} et Status : déconnecté`);

        req.session.destroy((err) => {
            if (err) return res.status(400).json({ msg: "Impossible de se déconnecter" });
            res.status(200).json({ msg: "Vous êtes déconnecté", uuid, name });
        });
    } catch (error) {
        res.status(500).json({ msg: "Erreur serveur" });
    }
