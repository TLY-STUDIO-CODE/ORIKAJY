...
try {
        ...
        if(req.role === "admin"){
            response = await Vente.findAll({
                attributes:['uuid', 'name', 'name_client', 'description', 'qte', 'categories', 'montant', 'date_vente', 'num_factV'],
                include:[{
                    model: User, 
...
export const createVente = async (req, res) => {
    ...
    try {
        ...
        const vente = await Vente.create({
            name, description, categories, qte, montant, name_client, date_vente, num_factV, stockId, userId: req.userId
        });
...
export const updateVente = async (req, res) => {
    try {
        ...
        await Vente.update({ name, description, categories, qte, montant, name_client, date_vente, num_factV, stockId },{
                where:{
                    id: vente.id
                }
            });
        ...
        await Transaction.update(
            {description_transaction, montant_transaction, type_transaction, date_transaction},
