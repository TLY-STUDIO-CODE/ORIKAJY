...
try {
        ...
        if(req.role === "admin"){
            response = await Vente.findAll({
                attributes:['uuid', 'name', 'name_client', 'qte', 'categories', 'montant', 'date_vente', 'num_fact'],
                include:[{
                    model: User, 
...
export const createVente = async (req, res) => {
    const { name, description, categories, qte, montant, name_client, date_vente, num_fact, stockId } = req.body;
    try {
        // Create the vente entry
        const vente = await Vente.create({
            name, description, categories, qte, montant, name_client, date_vente, num_fact, stockId, userId: req.userId
        });
...
export const updateVente = async (req, res) => {
    try {
        ...
        await Vente.update({ name, description, categories, qte, montant, name_client, date_vente, num_fact, stockId },{
                where:{
                    id: vente.id
                }
            });
